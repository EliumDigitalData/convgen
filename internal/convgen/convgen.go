...
import (
	...
	"github.com/EliumDigitalData/convgen/internal/convgen/assign"
...
func (cg *Convgen) frameCode() []byte {
	...
	fmt.Fprintf(&buf, "// Code generated by github.com/EliumDigitalData/convgen%s. DO NOT EDIT.\n", versionSuffix)
	fmt.Fprintf(&buf, "package %s\n", cg.p.Pkg().Name)

	if imports := cg.w.Imports(); len(imports) != 0 {
		fmt.Fprintf(&buf, "import (\n")
		// Emit imports in a stable order. Ranging over the map directly would use
		// Go's randomized map iteration order; gofmt normally re-sorts imports
		// afterward, but sorting here keeps the output deterministic even if
		// format.Source fails.
		aliases := slices.Sorted(maps.Keys(imports))
		for _, alias := range aliases {
			imp := imports[alias]
			// Check for remaining convgen import
			if imp.Path() == "github.com/EliumDigitalData/convgen" {
